...
# repo-layout fix: this repo keeps YOLOv5 code under victim_detector/
if str(Path(__file__).resolve().parents[0]) not in sys.path:
    sys.path.insert(0, str(Path(__file__).resolve().parents[0]))
import victim_detector.models as _vd_models
import victim_detector.utils as _vd_utils
sys.modules['models'] = _vd_models
sys.modules['utils'] = _vd_utils
...
import val_patch
from pso import ADMSPSOConfig, OptimizeFunction, PSO
...
if __name__ == '__main__':
    ...
    parser.add_argument('--workers', type=int, default=2, help='max dataloader workers (per RANK in DDP mode); default lowered to 2 for Windows (8 can exhaust the commit limit)')
    parser.add_argument('--noval', action='store_true', help='only validate final epoch')
    parser.add_argument('--epochs', type=int, default=3)
    parser.add_argument('--label-smoothing', type=float, default=0.0, help='Label smoothing epsilon')
    
    parser.add_argument('--patch_size', type=float, default=0.14)
    parser.add_argument('--swarm-size', type=int, default=30, help='ADMS-PSO particle count')
    parser.add_argument('--pso-iterations', type=int, default=100, help='ADMS-PSO iterations per optimisation call')
    parser.add_argument('--pso-seed', type=int, default=None, help='optional deterministic ADMS-PSO seed')
    ...
    weights = str(Path(__file__).resolve().parent / "model_trained_on_infrared_dataset.pt")
    ...
    # PSO
    pso_config = ADMSPSOConfig(swarm_size=opt.swarm_size,
                               iterations=opt.pso_iterations,
                               seed=opt.pso_seed)
    pso = PSO(opt.swarm_size, device, config=pso_config)
    func = OptimizeFunction(model, opt.patch_size, device, config=pso_config)
